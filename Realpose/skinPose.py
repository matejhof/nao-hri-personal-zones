      ...
      while sharedIsrunning:
         ...
         if movement['names'] != [] and timeout <= 0:
            ...
            if "HeadYaw" in movement["names"]:
               self.config.motion.setAngles("HeadYaw", self.config.motion.getAngles("HeadYaw", True), 0.05) 
            if "HeadPitch" in movement["names"]:
               self.config.motion.setAngles("HeadPitch", self.config.motion.getAngles("HeadPitch", True), 0.05)
...
class start:
   ...
   def getPosition(self, joint):
      ...
      if self.config.motion != None:
         return self.config.motion.getPosition(joint, 0, True)
      else:
         return [0, 0, 0, 0, 0, 0]
